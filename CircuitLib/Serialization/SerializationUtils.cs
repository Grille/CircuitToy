...
{
    public static void WriteEntities(this BinaryViewWriter bw, IList<Entity> entities)
    {
        ...
        bw.WriteWiresIndices(wires, nodes, netPins);

    }

    public static void WriteCircuit(this BinaryViewWriter bw, Circuit circuit, List<Type> types)
    {
        var nodes = circuit.Nodes;
        var networks = circuit.Networks;

        bw.WriteNodes(nodes, types);
        ...
        for (int i = 0; i < networks.Count; i++)
        {
            ...
            bw.WriteNetwork(network, nodes);
        }

    }

    public static void WriteNetwork(this BinaryViewWriter bw, Network network, IList<Node> nodes)
    {
        ...
        bw.WriteWiresIndices(wires, nodes, network.Pins.NetPins);

    }

    public static void WriteWiresIndices(this BinaryViewWriter bw, IList<Wire> wires, IList<Node> nodes, IList<WirePin> netPins)
    {
    ...
    public static List<Type> WriteNodeTypes(this BinaryViewWriter bw, Node node)
    {
        return bw.WriteNodeTypes(new[] { node });
    }
    public static List<Type> WriteNodeTypes(this BinaryViewWriter bw, IList<Node> nodes)
    {
    ...
    public static void WriteNodes(this BinaryViewWriter bw, IList<Node> nodes, List<Type> types)
    {
        bw.Write(nodes.Count);
        for (int i = 0; i < nodes.Count; i++)
        {
            ...
            bw.WriteNode(node, types, Vector2.Zero);
        }
    }

    public static void WriteNode(this BinaryViewWriter bw, Node node)
    {
        var types = WriteNodeTypes(bw, node);
        bw.WriteNode(node, types, Vector2.Zero);
    }

    public static void WriteNode(this BinaryViewWriter bw, Node node, List<Type> types, Vector2 offset)
    {
