...
public partial class CircuitEditor
{
        ...
        set {
            ...
            switch (_mode)
            {
                case ToolMode.SelectAndMove:
                    UserTool = new UserToolSelectAndMove(this);
                    break;
                case ToolMode.AddWire:
                    UserTool = new UserToolAddWire(this);
                    break;
                case ToolMode.OnOff:
                    UserTool = new UserToolOnOff(this);
                    break;
            }
        }
    ...
    public UserActionHandler UserTool;
    ...
    public void Paste(byte[] buffer)
    {
        ...
        foreach (var obj in selection)
        {
            ...
            if (obj is WirePin netpin)
            {
                netpin.Owner.Reset(State.Off);
            }
            if (obj is Node node)
            {
    ...
    public void InvokeMouseDown(EditorMouseArgs args)
    {
        WorldMouseDownPos = WorldMousePos;

        UserTool.MouseDown(args);
    }

    public void InvokeMouseMove(EditorMouseArgs args)
    {
        ScreenMousePos = args.Location;
        WorldMousePos = Camera.ScreenToWorldSpace(args.Location);

        UserTool.MouseMove(args);
        //if (doHover)
        //    Selection.HoverAt(WorldMousePos);
    }

    public void InvokeMouseUp(EditorMouseArgs args)
    {
        WorldMouseUpPos = WorldMousePos;

        UserTool.MouseUp(args);
    }
}
